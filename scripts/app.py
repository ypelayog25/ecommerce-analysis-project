...
Professional interactive dashboard with KPIs, charts, filters, and download options.
"""
...
# -----------------------
# Verify dataset columns (non-blocking)
# -----------------------
try:
...
with tab1:
    ...
    st.plotly_chart(style_fig(fig_country, theme), use_container_width=True)
    ...
    fig_monthly = px.line(monthly, x='order_date', y='total_price', markers=True,
                          title="Monthly Revenue Trend", text='total_price')
    st.plotly_chart(style_fig(fig_monthly, theme), use_container_width=True)
...
with tab2:
    ...
    st.plotly_chart(style_fig(fig_customers, theme), use_container_width=True)
...
with tab3:
    ...
    st.plotly_chart(style_fig(fig_products, theme), use_container_width=True)
...
with tab4:
    ...
    st.plotly_chart(style_fig(fig_price, theme), use_container_width=True)
