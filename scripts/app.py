...
if os.path.exists(dataset_parquet):
    ...
    st.error("❌ Dataset not found. Please run the feature engineering workflow first.")
    st.stop()
...
if missing:
    st.warning(f"⚠️ Missing required columns: {missing}. Some charts may not render.")
    st.stop()
